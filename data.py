...
for record in root.findall('.//data/record'):
    ...
    if country == 'India' and item == 'Population, total':
        data.append({'Year': int(year), 'Population': int(value)})

# Convert to DataFrame
df = pd.DataFrame(data)

# Plot 1: Bar chart (Population over the years for Aruba)
plt.figure(figsize=(14, 6))
plt.bar(df['Year'], df['Population'], color='skyblue')
plt.title('Total Population of India Over the Years')
plt.xlabel('Year')
...
plt.title('Distribution of India\'s Population (1960–2024)')
